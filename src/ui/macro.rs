...
impl eframe::App for Macro {
    fn update(&mut self, ctx: &Context, _frame: &mut Frame) {
        ...
        CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.label("R6 Macro");
                ui.radio_value(&mut self.lang, Lang::Chinese, "中文");
                ui.radio_value(&mut self.lang, Lang::English, "English");
            });
            ui.separator();
            ComboBox::from_label(match self.lang {
                Lang::English => "Weapon",
                Lang::Chinese => "主手武器",
            })
            ...
            ComboBox::from_label(match self.lang {
                ...
                .selected_text(match self.lang {
                    Lang::English => self.weapon.sight.to_eng(),
                    Lang::Chinese => self.weapon.sight.to_str(),
                })
                .show_ui(ui, |ui| {
                    ui.style_mut().wrap = Some(false);
                    for sight in crate::weapon::Sight::value_variants() {
                        if ui.selectable_value(&mut self.weapon.sight, *sight, match self.lang {
                            Lang::English => sight.to_eng(),
                            Lang::Chinese => sight.to_str(),
                        }).changed() {
                            if let Some(value) = self.config.map.get(&self.weapon) {
                                self.level = *value;
                            }
            ...
            ComboBox::from_label(match self.lang {
                ...
                .selected_text(match self.lang {
                    Lang::English => self.weapon.barrel.to_eng(),
                    Lang::Chinese => self.weapon.barrel.to_str(),
                })
                .show_ui(ui, |ui| {
                    ui.style_mut().wrap = Some(false);
                    for barrel in crate::weapon::Barrel::value_variants() {
                        if ui.selectable_value(&mut self.weapon.barrel, *barrel, match self.lang {
                            Lang::English => barrel.to_eng(),
                            Lang::Chinese => barrel.to_str(),
                        }).changed() {
                            if let Some(value) = self.config.map.get(&self.weapon) {
                                self.level = *value;
                            }
            ...
            ComboBox::from_label(match self.lang {
                ...
                .selected_text(match self.lang {
                    Lang::English => self.weapon.grip.to_eng(),
                    Lang::Chinese => self.weapon.grip.to_str(),
                })
                .show_ui(ui, |ui| {
                    ui.style_mut().wrap = Some(false);
                    for grip in crate::weapon::Grip::value_variants() {
                        if ui.selectable_value(&mut self.weapon.grip, *grip, match self.lang {
                            Lang::English => grip.to_eng(),
                            Lang::Chinese => grip.to_str(),
                        }).changed() {
                            if let Some(value) = self.config.map.get(&self.weapon) {
                                self.level = *value;
                            }
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button(if self.current_name.eq(&self.editing_name) {
                    match self.lang {
                        Lang::English => "Overwrite Favorite",
                        Lang::Chinese => "覆盖收藏",
                    }
                } else {
                    match self.lang {
                        Lang::English => "Add Favorite",
                        Lang::Chinese => "添加收藏",
                    }
                }).clicked() {
            ...
            ui.horizontal(|ui| {
                ComboBox::from_label(match self.lang {
                    Lang::English => "Favorite List",
                    Lang::Chinese => "收藏列表",
                }).selected_text(self.current_name.as_str()).show_ui(ui, |ui| {
                ...
                if ui.button(match self.lang {
                    Lang::English => "Remove Current Favorite",
                    Lang::Chinese => "删除当前收藏",
                }).clicked() {
            ...
            ui.label(match self.hand {
                Hand::Main => match self.lang {
                    Lang::English => "ON",
                    Lang::Chinese => "已启用",
                },
                Hand::Deputy => match self.lang {
                    Lang::English => "OFF",
                    Lang::Chinese => "已禁用",
                },
            });
            ui.separator();
            ui.label(match self.lang {
                Lang::English => "+/- Modify Pressure Value",
                Lang::Chinese => "+/- 调整下压值",
            });
            ui.label(match self.lang {
                Lang::English => "1/2 ON/OFF Macro",
                Lang::Chinese => "1/2 启用/禁用宏",
            });
            ui.label(match self.lang {
                Lang::English => "First press the right button and then the left button to start pressure",
                Lang::Chinese => "先按住右键再按住左键开始下压",
            });
            ui.label(match self.lang {
                Lang::English => "Please add commonly used configurations to favorites",
                Lang::Chinese => "请把常用的配置添加到收藏",
            });
        });
